...
const ApplicantsTable = () => {
  ...
  const statusHandler = async (status, id) => {
    try {
      axios.defaults.withCredentials = true; //This tells Axios to include cookies/session tokens in every request globally.
  ...
  return (
    <div>
      <Table>
        ...
        <TableBody className="font-400 text-[16px]">
          {applicants &&
            applicants?.applications?.map((item) => (
              <tr key={item._id}>
                <TableCell>{item?.applicant?.fullname}</TableCell>
                <TableCell className="text-green-400">{item?.applicant?.email}</TableCell>
