...
const Job = ({ job }) => {
  const navigate = useNavigate();
   ...
   const daysAgoFunction = (mongodbTime) => {             //It calculates how many days have passed since the job was posted.
  ...
  return (
    <div className="px-6 py-2 rounded-2xl shadow-md bg-white border border-gray-200 hover:shadow-lg transition-shadow duration-300 flex flex-col justify-between min-h-[260px]">
      <div className="text-xs text-gray-400  text-right">
        {daysAgoFunction(job?.createdAt) === 0 ? "Today" : `${daysAgoFunction(job?.createdAt)} day(s) ago`}   {/*Date posted  */}
      </div>

      <div className="flex items-center gap-3 mb-3">
        <Avatar className="w-12 h-12 border">
          <AvatarImage src={job?.company?.logo || "https://via.placeholder.com/40"} alt="Company Logo" />     {/* Company Info */}
        </Avatar>
        <div>
      ...
      <div className="mb-3">                                                                            {/* Job Title and Description */}
      ...
      <div className="flex flex-wrap gap-2 mb-2">                                                                 {/* Tags / Badges */}
        ...
        <Button onClick={() => navigate(`/description/${job?._id}`)} variant="secondary" className="w-full">         {/* View Details Button */}
          View Details
        </Button>
      </div>
    </div>
